    ...
    <section className="workspace">
      ...
      <section className="stage panel">
        <div className={`canvas ${dragging ? "dragging" : ""}`} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop}>
          <div className="stage-background-controls" aria-label="Фон сцены"><span>Фон сцены</span><div className="background-picker"><button className={sceneBackground === "light" ? "active" : ""} onClick={() => setSceneBackground("light")}><i className="light-swatch"/>Светлый</button><button className={sceneBackground === "dark" ? "active" : ""} onClick={() => setSceneBackground("dark")}><i className="dark-swatch"/>Тёмный</button></div></div>
      ...
      <aside className="controls panel"><div className="panel-title"><span>Пресеты</span><em>{appliedCount}</em></div><div className="controls-scroll"><div className="selection-name"><small>Выбранная анимация</small><b>{selected.includes("__root") ? "Вся иконка" : selected.length > 1 ? `${selected.length} слоя` : layers.find(l => l.id === selected[0])?.label || "Слой"}</b></div>
        ...
        </div> : <p className="preset-empty">Выберите пресет, чтобы увидеть его настройки</p>}</div>
